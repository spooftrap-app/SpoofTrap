...
final class RobloxLogWatcher: ObservableObject {
    ...
    private nonisolated static func readProcessMemory() -> String? {
        ...
        guard let pidStr = String(data: pidData, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines),
              ...
              !pid.isEmpty,
              pid.range(of: "^[0-9]+$", options: .regularExpression) != nil else { // 🛡️ Sentinel: Prevent argument injection
            return nil
        }
    ...
    private func resolveGameName(placeId: String) {
        guard placeId.range(of: "^[0-9]+$", options: .regularExpression) != nil else { return } // 🛡️ Sentinel: Prevent SSRF
        Task.detached {
    ...
    private func resolveRegion(ip: String) {
        guard ip.range(of: "^[a-fA-F0-9.:]+$", options: .regularExpression) != nil else { return } // 🛡️ Sentinel: Prevent SSRF
        Task.detached {
    ...
    private nonisolated static func measurePing(ip: String?) -> Int? {
        guard let ip, !ip.isEmpty, ip.range(of: "^[a-fA-F0-9.:]+$", options: .regularExpression) != nil else { return nil } // 🛡️ Sentinel: Prevent argument injection
        let process = Process()
        let pipe = Pipe()
