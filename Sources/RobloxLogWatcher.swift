...
final class RobloxLogWatcher: ObservableObject {
    ...
    private nonisolated static func readProcessMemory() -> String? {
        ...
        guard let pidStr = String(data: pidData, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines),
              ...
              !pid.isEmpty,
              pid.range(of: "^[0-9]+$", options: .regularExpression) != nil else { // 🛡️ Sentinel: Validate PID to prevent command injection
            return nil
        }
    ...
    private nonisolated static func measurePing(ip: String?) -> Int? {
        guard let ip, !ip.isEmpty,
              ip.range(of: "^[a-fA-F0-9.:]+$", options: .regularExpression) != nil else { return nil } // 🛡️ Sentinel: Validate IP to prevent command injection
        let process = Process()
        let pipe = Pipe()
