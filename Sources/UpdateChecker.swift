...
final class UpdateChecker: ObservableObject {
    ...
    private func relaunchApp() {
        ...
        // 🛡️ Sentinel: Prevent command injection by passing appPath securely as an argument instead of interpolating it directly into the shell command
        task.arguments = ["-c", "sleep 1 && /usr/bin/open \"$1\"", "--", appPath]
        try? task.run()
